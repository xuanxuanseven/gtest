...
class Slide{
    ...
    */
    private function init_core(){
        ...
        //  背景图片存放目录 （Resources）
        $this->ResourcesPath = APPLICATION_PATH . '/application/extends/verify/img/'  ;
        //$this->ResourcesPath = '/extends/verify/img/';
    ...
    */
    protected function restruct_cut(){
       ...
       self::$session->set(self::$session_name, $this->location_x);

       // 
       $bg_pic = ltrim($bigPicName,'.');
       $bg_pic = substr($bg_pic, 38);
       $ico_pic_url = ltrim($this->smallPicName,'.');
       $ico_pic_url = substr($ico_pic_url, 38);

        
        $this->get_png_info = array(
            'data'    => $_temp_xy_data,
            'bg_pic'  =>  $bg_pic,//ltrim($bigPicName,'.'),
            'ico_pic' => array('url'=> $ico_pic_url,'w'=> $this->tailoring_w ,'h'=>$this->tailoring_h),
