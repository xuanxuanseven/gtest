...
function msgmerge($outfile, $data)
{
    ...
    passthru('msgcat -o ' . escapeshellarg($tmp2) . ' ' . escapeshellarg($outfile) . ' ' . escapeshellarg($tmp) . '  --force-po --sort-output',
             $rc);
    if ($rc) {
...
function do_file($outfile, $file)
{
    ...
    for ($i = 0; $i < count($matches[0]); $i++) {
        if (preg_match('/context\s*=\s*["\']?\s*(.[^\"\']*)\s*["\']?/', $matches[2][$i][0], $match_context)) {
            $msgid = $matches[3][$i][0];
            //带上下文的
            if (preg_match('/plural\s*=\s*["\']?\s*(.[^\"\']*)\s*["\']?/', $matches[2][$i][0], $match)) {
                $msgids_context_plural[$match_context[1]][$msgid] = $match[1];
            }
            $lineno = lineno_from_offset($content, $matches[2][$i][1]);
            $msgids_context[$match_context[1]][$msgid][] = "$file:$lineno";
        } else {
            //没上下文
            if (preg_match('/plural\s*=\s*["\']?\s*(.[^\"\']*)\s*["\']?/', $matches[2][$i][0], $match)) {
                $msgid = $matches[3][$i][0];
    ...
    ob_start();
    echo MSGID_HEADER;
    foreach ($msgids_context as $context_name => $item) {
        foreach ($item as $msgid => $files) {
            echo "#: ", join(' ', $files), "\n";
            echo 'msgctxt "' . fs($context_name) . '"' . "\n";
            if (isset($msgids_context_plural[$context_name][$msgid])) {
                echo 'msgid "' . fs($msgid) . '"', "\n";
                echo 'msgid_plural "' . fs($msgids_context_plural[$context_name][$msgid]) . '"', "\n";
                echo 'msgstr[0] ""', "\n";
                echo 'msgstr[1] ""', "\n";
            } else {
                echo 'msgid "' . fs($msgid) . '"', "\n";
