...
class Lephp extends BaseLephp
{
    ...
    public static function run($defaultVersion = 'v1')
    {
         ...
         * 加载默认配置文件
         */
        require APPLICATION_PATH . '/conf/define.php';//moduleConfig 是在这里赋值的
    ...
    private static function loadConfig()
    {
         ...
         * 根据访问路径加载不同的配置文件
         * 优先级:
         * 如请求 url path 为 a/b/c/d
         * 那配置文件优先级为:
         * /conf/a_b_c_d/a_b_c_d.php > /conf/a_b_c/a_b_c.php > /conf/a_b/a_b.php > /conf/d/d.php > /conf/c/c.php > /conf/b/b.php > conf/a/a.php
         *
         */
        $request = new Request();
        ...
        $joinedPathArray = [];
        foreach ($pathArray as $atomPath) {
            if (file_exists(APPLICATION_PATH . "/conf/{$atomPath}/{$atomPath}.php")) {
                Loader::import(APPLICATION_PATH . "/conf/{$atomPath}/{$atomPath}.php");
            }

            if (!empty($joinedPathArray)) {
                $joinedPathArray[] = end($joinedPathArray) . "_" . $atomPath;
            }
        }
        foreach ($joinedPathArray as $atomPath) {
            if (file_exists(APPLICATION_PATH . "/conf/{$atomPath}/{$atomPath}.php")) {
                Loader::import(APPLICATION_PATH . "/conf/{$atomPath}/{$atomPath}.php");
            }
        }
        ...
        if (count($explodedDomain) > 3) {
            if (file_exists(APPLICATION_PATH . "/conf/{$explodedDomain[0]}/{$explodedDomain[0]}.php")) {
                Loader::import(APPLICATION_PATH . "/conf/{$explodedDomain[0]}/{$explodedDomain[0]}.php");
            }
        }
    }
