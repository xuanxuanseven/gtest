...
class NewQuery extends Component implements QueryInterface
{
    use QueryTrait;
     ...
     * @param NewConnection $db the database connection used to generate the SQL statement.
     ...
     * @param NewConnection $db the database connection. If not set, the "db" application component will be used.
     * @return BatchQueryResult|object
     * and can be traversed to retrieve the data in batches.
     */
    public function batch($batchSize = 100, $db = null)
    {
        return Lephp::createObject([
    ...
    private function queryScalar($selectExpression, $db)
    {
        ...
        if (empty($this->groupBy) && empty($this->union) && !$this->distinct) {
            ...
            return (new NewQuery)->select([$selectExpression])
                ->from(['c' => $this])
                ->createCommand($db)
                ->queryScalar();
        }
     ...
     * @param array $params the parameters (name => value) to be bound to the query.
     * @return NewQuery the query object itself
     */
     ...
     * @param NewQuery $from the source query object
     * @return NewQuery the new Query object
     */
