...
use Lephp\Core\Loader;
use Lephp\Exception\DbException;
use PDO;
...
class Query
{
    ...
    protected function builder()
    {
        ...
        if (!isset($builder[$driver])) {
            $class = '\\Lephp\\Db\\Builder\\' . ucfirst($driver);
     ...
     * @param mixed $op 查询表达式 lt eq gt 或者 < = >
...
//            $info  = $class->getRelationInfo();
    ...
    public function select($data = [])
    {
        ...
        if (!$resultSet) {
            ...
            // 执行查询操作
            $resultSet = $this->connection->query($sql, $this->getBind(), $options['fetch_sql'], $options['master'],
                                                  $options['fetch_pdo']);
    ...
    public function find($data = [])
    {
        ...
        if (!$result) {
            ...
            $sql = $this->builder()->select($options);
            // 执行查询
            $result = $this->connection->query($sql, $this->getBind(), $options['fetch_sql'], $options['master'],
                                               $options['fetch_pdo']);
