...
{
    /**
     * @var NewConnection the database connection that this transaction is associated with.
     */
    public $db;
    /**
    ...
    public function begin()
    {
        ...
        if ($this->_level == 0) {
            Lephp::trace('Begin transaction', __METHOD__);
        ...
        if ($schema->supportsSavepoint()) {
            Lephp::trace('Set savepoint ' . $this->_level, __METHOD__);
            $schema->createSavepoint('LEVEL' . $this->_level);
        } else {
            Lephp::info('Transaction not started: nested transaction not supported', __METHOD__);
        }
        $this->_level++;
    }
    ...
    public function commit()
    {
        ...
        $this->_level--;
        if ($this->_level == 0) {
            Lephp::trace('Commit transaction', __METHOD__);
            $this->db->pdo->commit();
        ...
        $schema = $this->db->getSchema();
        if ($schema->supportsSavepoint()) {
            Lephp::trace('Release savepoint ' . $this->_level, __METHOD__);
            $schema->releaseSavepoint('LEVEL' . $this->_level);
        } else {
            Lephp::info('Transaction not committed: nested transaction not supported', __METHOD__);
        }
    }
    ...
    public function rollBack()
    {
        ...
        $this->_level--;
        if ($this->_level == 0) {
            Lephp::trace('Roll back transaction', __METHOD__);
        ...
        $schema = $this->db->getSchema();
        if ($schema->supportsSavepoint()) {
            Lephp::trace('Roll back to savepoint ' . $this->_level, __METHOD__);
            $schema->rollBackSavepoint('LEVEL' . $this->_level);
        } else {
            Lephp::info('Transaction not rolled back: nested transaction not supported', __METHOD__);
