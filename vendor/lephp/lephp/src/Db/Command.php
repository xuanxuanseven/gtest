...
{
    /**
     * @var NewConnection the DB connection that this command is associated with
     */
    public $db;
    /**
    ...
    private function queryInternal($method, $fetchMode = null)
    {
        ...
        Lephp::info($rawSql, 'Lephp\Db\Command::query');
        //todo  read from cache results
        $token = $rawSql;
        try {
            Lephp::beginProfile($token, 'Lephp\Db\Command::query');
            $this->prepare();
            $this->pdoStatement->execute();
            if ($method === '') {
            ...
            Lephp::endProfile($token, 'Lephp\Db\Command::query');
            //todo cache results
     ...
     * @param string $type the column type. [[\Lephp\Db\QueryBuilder::getColumnType()]] will be called
