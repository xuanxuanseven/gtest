...
abstract class Builder
{
     ...
     * 架构函数
     * @access public
     * @param \Db\Connection $connection 数据库连接对象实例
     */
     ...
     * 设置当前的Query对象实例
     * @access protected
     * @param \lephp\db\Query $query 当前查询对象实例
     * @return void
     */
    ...
    {
        if (is_string($value)) {
            $value = strpos($value, ':') === 0 && $this->query->isBind(substr($value,
                                                                              1)) ? $value : $this->connection->quote($value);
