...
class NewConnection extends Component
{
    ...
    /**
     * @var \Lephp\Cache\Dependency the dependency that will be used when saving query results into cache.
     ...
     */
    public $schemaMap = [
        'mysql'   => 'Lephp\Db\Mysql\Schema',    // MySQL
        //        'sqlite'  => 'Lephp\Db\sqlite\Schema',  // sqlite 3
        //        'sqlite2' => 'Lephp\Db\sqlite\Schema', // sqlite 2
    ];
    /**
     ...
     * @param \Lephp\Cache\Dependency $dependency the dependency for the cached query result.
    ...
    public function open()
    {
        if ($this->pdo === null) {
            ...
            try {
                Lephp::trace($token, __METHOD__);
                Lephp::beginProfile($token, __METHOD__);
                $this->pdo = $this->createPdoInstance();
                $this->initConnection();
                Lephp::endProfile($token, __METHOD__);
    ...
    {
        if ($this->pdo !== null) {
            Lephp::trace('Closing DB connection: ' . $this->dsn, __METHOD__);
            $this->pdo = null;
            $this->_schema = null;
     ...
     * @throws \Lephp\Core\NotSupportedException if there is no support for the current driver type
     */
    public function getSchema()
    {
        if ($this->_schema !== null) {
            ...
            if (isset($this->schemaMap[$driver])) {
                ...
                return $this->_schema = Lephp::createObject($config);
            } else {
